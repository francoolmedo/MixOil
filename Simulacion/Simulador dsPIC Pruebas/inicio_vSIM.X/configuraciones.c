...
void inicializaDMA() {
    ...
    /*CONFIGURACION DMA  Transmicion*/
    DMA0CON = 0x2001; // One-Shot, Post-Increment, RAM-to-Peripheral
    DMA0CNT = (TAM_DMA - 1); //CNT=7 PARA  8 DMA requests
    DMA0REQ = 0x001F; // Fuente de solicitud de transmisión UART1 (U1TX)
    //DMA0REQ = 0x000C; // Select UART1 Transmitter
    //DMA0PAD = (volatile unsigned int) &U1TXREG;
    DMA0PAD = (volatile unsigned int) &U1TXREG; // Dirección del registro U1TXREG para la transmisión   
