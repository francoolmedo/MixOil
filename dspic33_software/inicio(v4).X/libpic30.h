...
#define _strlen_eds(S) ((__eds__ char*)_memchr_eds((S),0,0xFFFFFF) - (S))
#define _strnlen_eds(S,maxlen) ((__eds__ char*)_memchr_eds((S),0,maxlen) - (S))
...
#if defined(__dsPIC30F__) || defined(__PIC24H__)
...
#else
/* Prototypes that will print deprecated warning message */
extern void _flash_helper1 (_prog_addressT dst, int code)
...
extern int  _program_secondary(int core_number, int verify,
                               __eds__ unsigned char *image);

extern int  _program_inactive_secondary(int core_number, int verify,
                                        __eds__ unsigned char *image);

extern int _wipe_secondary(int core_number);
